                          ...
                          asn1_parser_tag_t *parent)
{
    enum
    {
        ASN1_MASK_MULTI = 0x20,
        ASN1_MASK_TYPE = 0x1F,
        ASN1_MASK_CLASS = 0xC0,
        ASN1_SHIFT_CLASS = 6
    };
    asn1_parser_tag_t tag[1];
    int rc;
...
int gost_compute_public(EC_KEY *ec)
{
    ...
    res = EC_POINT_mul(group, pub_key, priv_key, 0, 0, ctx);
    if (!res)
    {
        ERROR("ec mul\n");
        rc = 5;
    ...
    res = EC_KEY_set_public_key(ec, pub_key);
    if (!res)
    {
        ERROR("set pub key");
        rc = 6;
                                     ...
                                     char *pub8, int key_size, BN_CTX *ctx, int ecparam_id)
{
    ...
    char buf[max_key_size], pub[max_key_size];
    int res, rc;
    EC_KEY *eckey = 0;
    ...
    res = fill_GOST_EC_params(eckey, ecparam_id);
    if (!res)
    {
        ERROR("set ec param");
        rc = 8;
    ...
    res = EC_GROUP_get_order(group, order, ctx);
    if (!res)
    {
        ERROR("no group order");
        rc = 10;
        goto err;
    }
    mask_inv = BN_mod_inverse(mask_inv, mask, order, ctx);
    if (!mask_inv)
    {
        ERROR("inverse fail");
        rc = 11;
    ...
    res = BN_mod_mul(raw_secret, key_with_mask, mask_inv, order, ctx);
    if (!res)
    {
        ERROR("bn mul");
        rc = 12;
    ...
    res = EC_KEY_set_private_key(eckey, raw_secret);
    if (!res)
    {
        ERROR("ec set priv key");
        rc = 13;
        goto err;
    }
    rc = gost_compute_public(eckey);
    if (rc)
    {
        ERROR("compute pub");
        rc = 14;
    ...
    res = EC_POINT_get_affine_coordinates_GFp(group, pubkey, x, y, ctx);
    if (!res)
    {
        ERROR("ec get coord");
        rc = 16;
...
{
    char pwd_key[max_key_size], buf[max_key_size];
    int rc, ec = 0;
    BN_CTX *ctx;
    BIGNUM *key_with_mask, *mask, *raw_key;
    ...
    {
        rc = make2001_pwd_key(pwd_key, data->salt, 12, data->password);
        ec = 0;
        if (rc)
        {
            ERROR("pass");
    ...
    else if (data->alg == ALG_GOST2012_256)
    {
        rc = make2012_pwd_key(pwd_key, data->salt, 12, data->password);
        ec = 1;
        if (rc)
        {
            ERROR("pass");
    ...
    else if (data->alg == ALG_GOST2012_512)
    {
        rc = make2012_pwd_key(pwd_key, data->salt, 12, data->password);
        ec = 1;
        if (rc)
...
int get_cpcert(const char *path, const char *pass)
{
    ...
    if (pkt_readfile(header, fn))
    {
        ERROR("read header");
        rc = 25;
        goto err;
    }
    if (asn1_parse(header, cfg))
    {
        ERROR("parse header");
        rc = 26;
        goto err;
    ...
    if (pkt_readfile(primary, fn))
    {
        ERROR("read primary");
        rc = 27;
        goto err;
    }
    if (asn1_parse(primary, cfg))
    {
        ERROR("parse primary");
        rc = 28;
        goto err;
    ...
    if (pkt_readfile(masks, fn))
    {
        ERROR("read masks");
        rc = 29;
        goto err;
    }
    if (asn1_parse(masks, cfg))
    {
        ERROR("parse masks");
        rc = 30;
        goto err;
    ...
    {
        ERROR("unable to get privkey");
        rc = 31;
        goto err;
    }
    if (print_cert(data, stdout))
    {
        ERROR("print");
        rc = 32;
        goto err;
